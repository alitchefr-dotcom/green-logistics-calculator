...
T = {
    "title": "⚡ Renewable Energy Logistics & Landed Cost Calculator" if not is_hebrew else "⚡ Renewable Energy Logistics & Landed Cost Calculator",
    ...
    "tab5": "🗺️ Route Comparison" if not is_hebrew else "🗺️ השוואת מסלולי נמלים",
    "tab6": "📊 Landed Cost Summary" if not is_hebrew else "📊 Landed Cost & Summary",
...
with tab1:
    ...
    with col1:
        ...
        dest_country = st.selectbox(T["dest_country"], list(VAT_RATES.keys()), index=default_country_idx)
    ...
    with col2:
        ...
        bess_mwh = st.number_input(T["bess_capacity"], value=40.0, step=5.0, help="Total MWh for $/kWh logistics cost metric" if not is_hebrew else "קיבולת אגירה ב-MWh לצורך חישוב מדד העלות ל-kWh")
...
with tab3:
    ...
    with col_y:
        ...
        ext_drayage_cost = st.number_input(T["inland_drayage"], value=default_cross_border_drayage, step=50.0, help="Inland trucking from discharge port to site" if not is_hebrew else "הובלה יבשתית מנמל הפריקה עד לאתר הפרויקט")
...
# ----- T5: השוואת מסלולי נמלים (Burgas vs Constanta) -----
with tab5:
    st.subheader("🗺️ Port Route Optimization (Romania / Balkan Projects)" if not is_hebrew else "🗺️ ניתוח השוואתי: מסלול בורגס (בוליגריה) מול קונסטנצה (רומניה)")
    st.caption("Comparison for 10x BESS Containers to Romania Sites (Iepurești / Ghimpați)" if not is_hebrew else "השוואת עלויות שינוע עבור 10 מכולות BESS לאתרי פרויקט ברומניה")
    
    col_r1, col_r2 = st.columns(2)
    with col_r1:
        st.markdown("### 🇧🇬 Route A: via Burgas, Bulgaria")
        st.markdown("* **Ocean Freight:** ~$18,375 / unit")
        st.markdown("* **Cross-Border Drayage to Romania:** ~$1,850 / container")
        st.markdown("* **Key Advantage:** Lower port congestion, faster clearance for DG Class 9")
    with col_r2:
        st.markdown("### 🇷🇴 Route B: via Constanța, Romania")
        st.markdown("* **Ocean Freight:** ~$19,500 / unit")
        st.markdown("* **Local Inland Drayage:** ~$850 / container")
        st.markdown("* **Key Advantage:** Direct discharge in destination country, no cross-border customs transit")
